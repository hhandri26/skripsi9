...
<div class="toolbar-nav" >   
	<div class="row">
		<div class="col-md-8"> 	
			@if($access['is_add'] ==1)
			<a href="{{ url('bb/create?return='.$return) }}" class="btn  btn-sm"  
...
<div class="table-container">
			...
			<!-- Table Grid -->
 			...
 			{!! Form::open(array('url'=>'bb?'.$return, 'class'=>'form-horizontal m-t' ,'id' =>'SximoTable' )) !!}
		    ...
		    <table class="table  table-hover " id="{{ $pageModule }}Table">
		        ...
		        <tbody>        						
		            @foreach ($rowData as $row)
		                <tr>
							<td class="thead"> {{ ++$i }} </td>
							<td class="tcheckbox"><input type="checkbox" class="ids minimal-green" name="ids[]" value="{{ $row->menu_id }}" />  </td>
							<td>
							 	...
							 	<div class="dropdown">
								  ...
								  <ul class="dropdown-menu">
									...
									<li class="nav-item"><a href="{{ url('bb/'.$row->menu_id.'?return='.$return)}}" class="nav-link tips" title="{{ __('core.btn_view') }}"> {{ __('core.btn_view') }} </a></li>
									@endif
									@if($access['is_edit'] ==1)
									<li class="nav-item"><a  href="{{ url('bb/'.$row->menu_id.'/edit?return='.$return) }}" class="nav-link  tips" title="{{ __('core.btn_edit') }}"> {{ __('core.btn_edit') }} </a></li>
									@endif
									<div class="dropdown-divider"></div>
									@if($access['is_remove'] ==1)
										<li class="nav-item"><a href="javascript://ajax"  onclick="SximoDelete();" class="nav-link  tips" title="{{ __('core.btn_remove') }}">
